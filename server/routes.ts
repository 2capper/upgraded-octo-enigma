...
import { 
  ...
  tournaments,
  games,
  tournamentDiamondAllocations,
  insertTournamentDiamondAllocationSchema,
  type TournamentDiamondAllocation,
} from "@shared/schema";
...
`;
  ...
  // ============================================================================
  // TOURNAMENT DIAMOND ALLOCATIONS - "Tetris Schema" for multi-division scheduling
  // ============================================================================

  // Get all allocations for a tournament
  app.get("/api/tournaments/:tournamentId/allocations", async (req: any, res) => {
    try {
      const { tournamentId } = req.params;
  ...
  app.post("/api/tournaments/:tournamentId/allocations", requireAdmin, async (req: any, res) => {
    try {
      const { tournamentId } = req.params;
      ...
      // Validate and parse the allocation data
      const validatedData = insertTournamentDiamondAllocationSchema.parse({
        ...req.body,
        tournamentId,
      });
      
      // Check for overlapping allocations on same diamond/date
      const existingAllocations = await db
        ...
        .where(
          and(
            ...
            eq(tournamentDiamondAllocations.diamondId, validatedData.diamondId),
            eq(tournamentDiamondAllocations.date, validatedData.date)
          )
        );
      ...
      const newStart = parseTime(validatedData.startTime);
      const newEnd = parseTime(validatedData.endTime);
      ...
      const [newAllocation] = await db
        .insert(tournamentDiamondAllocations)
  ...
  app.put("/api/tournaments/:tournamentId/allocations/:allocationId", requireAdmin, async (req: any, res) => {
    try {
      const { tournamentId, allocationId } = req.params;
      
      // Verify allocation exists and belongs to this tournament
      const [existingAllocation] = await db
        .select()
        .from(tournamentDiamondAllocations)
        .where(
          and(
            eq(tournamentDiamondAllocations.id, allocationId),
            eq(tournamentDiamondAllocations.tournamentId, tournamentId)
          )
        );
      ...
      // Parse update data (allow partial updates)
      const updateData = {
      ...
      // Check for overlapping allocations (excluding self)
      const otherAllocations = await db
        .select()
        .from(tournamentDiamondAllocations)
        .where(
          and(
            eq(tournamentDiamondAllocations.tournamentId, tournamentId),
            eq(tournamentDiamondAllocations.diamondId, updateData.diamondId),
            eq(tournamentDiamondAllocations.date, updateData.date)
          )
        );
      ...
      const newStart = parseTime(updateData.startTime);
      const newEnd = parseTime(updateData.endTime);
      
      for (const existing of otherAllocations) {
        if (existing.id === allocationId) continue; // Skip self
      ...
      const [updatedAllocation] = await db
        ...
        .set(updateData)
        .where(eq(tournamentDiamondAllocations.id, allocationId))
        .returning();
  ...
  app.delete("/api/tournaments/:tournamentId/allocations/:allocationId", requireAdmin, async (req: any, res) => {
    try {
      const { tournamentId, allocationId } = req.params;
  ...
  // Bulk create allocations (for quick setup)
  app.post("/api/tournaments/:tournamentId/allocations/bulk", requireAdmin, async (req: any, res) => {
    try {
      const { tournamentId } = req.params;
  ...
  // Clear all allocations for a tournament
  app.delete("/api/tournaments/:tournamentId/allocations", requireAdmin, async (req: any, res) => {
    try {
      const { tournamentId } = req.params;
