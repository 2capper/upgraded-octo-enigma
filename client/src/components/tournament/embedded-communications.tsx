...
import { Send, MessageSquare, History, AlertCircle, CheckCircle, FileText, Edit, Trash2, Plus, Inbox, Mail, MailOpen, Target, Cloud, Calendar, Users, Zap, UserCheck } from "lucide-react";
...
interface EmbeddedCommunicationsProps {
  orgId: string;
  tournamentId: string;
...
export function EmbeddedCommunications({ orgId, tournamentId, tournamentName }: EmbeddedCommunicationsProps) {
  ...
  const { teams, pools, isLoading: tournamentLoading } = useTournamentData(tournamentId);

  const { data: rateLimit } = useQuery<{ allowed: boolean; remaining?: number; error?: string }>({
  ...
  const { data: inboundMessages = [], isLoading: inboxLoading } = useQuery<any[]>({
  ...
  if (tournamentLoading) {
    return <CommunicationsComposeSkeleton />;
  }
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="compose" className="space-y-4">
        <TabsList className="flex flex-wrap h-auto gap-1">
          <TabsTrigger value="compose" className="flex-shrink-0" data-testid="tab-compose">
            <Send className="h-4 w-4 mr-1" />
            Compose
          </TabsTrigger>
          <TabsTrigger value="templates" className="flex-shrink-0" data-testid="tab-templates">
            <FileText className="h-4 w-4 mr-1" />
            Templates
          </TabsTrigger>
          <TabsTrigger value="history" className="flex-shrink-0" data-testid="tab-history">
            <History className="h-4 w-4 mr-1" />
            History
          </TabsTrigger>
          <TabsTrigger value="inbox" className="flex-shrink-0" data-testid="tab-inbox">
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-sm">
                  Choose which staff members to message
                </CardDescription>
              </CardHeader>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" onClick={handleCreateTemplate} data-testid="button-create-template">
        ...
        <TabsContent value="inbox">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Messages received from coaches via SMS
              </CardDescription>
            </CardHeader>
            <CardContent>
              {inboxLoading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-muted-foreground mb-4">
                    When coaches text your number, messages appear here
                  </p>
                </div>
              ) : (
                <div className="space-y-2">
